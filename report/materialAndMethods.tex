...
This heuristic constrcuts the solution each row at a time, at each step selecting the most attractive remaining row, which is the one that maximizes the sum of elements having an influence on objective function.
...
This means the heursitic prefers the rows that have the highest remaining contribution to the objective function, so this method is intented to provide better intitial solutions than an uninformed random starting point.
